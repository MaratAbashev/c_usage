...
static long long copy_file_bytes(const char *src, const char *dst, mode_t mode)
{
    ...
    if (close(in) < 0 && total >= 0)
        total = -1;
    if (close(out) < 0 && total >= 0)
        total = -1;

    if (total >= 0)
    {
        if (chmod(dst, mode & 07777) != 0)
            return -1;
    }
    return total;
}
...
static int manager_work(const char *dir1, const char *dir2, int N)
{
    DIR *d = opendir(dir1);
    if (!d)
    {
...
int main(int argc, char **argv)
{
    ...
    if (mgr == 0)
    {
        int rc = manager_work(dir1, dir2, N);
        _exit(rc);
    }
